...
-- Create the single category for this exam
...
DECLARE
    cat_id bigint;
BEGIN
    SELECT id INTO cat_id FROM public.exam_categories WHERE name = 'GCP Practice Exam';
    
    -- Insert the 10 questions
    INSERT INTO public.questions (question_text, choice_a, choice_b, choice_c, choice_d, correct_answer, explanation, category_id) VALUES
    (
    ...
    (
        'You are rolling out a new load-testing tool for services on Compute Engine with Cloud Bigtable. Which requirements should be included?',
        ...
        'A',
        'Ensure load tests validate Cloud Bigtable performance using typical workloads to properly plan capacity and resource allocation.',
        cat_id
    ),
    (
    ...
    (
        'The company prioritizes release speed and agility. You want to reduce the chance of security errors being introduced. Which actions can you take?',
        ...
        'Run a vulnerability security scanner as part of CI/CD pipeline',
        'B',
        'Automated static analysis and vulnerability scanning in CI/CD reduces the risk of introducing security flaws without slowing development.',
        cat_id
    ),
    (
